 ...
 * example usage.
 */
public class TensorflowInferenceInterfaceModified implements TensorFlowInferenceInterfaceCustom {
    private static final String TAG = "TensorFlowInferenceInterface";
    protected static final String ASSET_FILE_PREFIX = "file:///android_asset/";
    ...
    public TensorflowInferenceInterfaceModified() {
        this.modelName = "";
        this.g = new Graph();
    ...
    public TensorflowInferenceInterfaceModified(AssetManager assetManager, String model) {
     ...
     */
    public TensorflowInferenceInterfaceModified(InputStream is) {
        prepareNativeRuntime();
        ...
        try {
            ...
            loadGraph(graphDef, g);
            is.close();
            Log.i(TAG, "Successfully loaded model from stream");

            if (VERSION.SDK_INT >= 18) {
                Trace.endSection(); // initializeTensorFlow.
            }
        } catch (IOException e) {
            throw new RuntimeException("Failed to load model from stream");
        }
    }
     ...
     */
    public TensorflowInferenceInterfaceModified(Graph g) {
        prepareNativeRuntime();
    ...
    protected void loadGraph(byte[] graphDef, Graph g) throws IOException {
